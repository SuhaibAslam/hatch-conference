...
First determine whether you can create 2 genuinely isolated contexts. If you can, run the baseline with only the task, input material and output format. Run the guided condition separately with the same material plus the agent skill. If you cannot guarantee isolation, return 2 ready-to-paste prompts for separate fresh conversations instead. Never expose the target skill to the baseline context and then call it clean.
...
When both outputs are available, help us place them side by side for Team Canvas 6 or the workshop Comparison Desk. Keep the four workshop review dimensions visible: task value, method and judgment, clarity and agency, and handling uncertainty. Leave scores, cited evidence and the skill improvement decision for our team.
